...
| `tools/specs.ts`         | The tool catalog as JSON-Schema specs (`TOOL_SPECS`). Each spec carries a `kind` (`read`/`create`/`update`/`delete`/`complete`/`uncomplete`/`convert`/`move`) used by the chat UI to pick icon/tone and to tell read-only from mutating tools (`TOOL_KIND_BY_NAME`, `toolKind`, `isReadonlyTool`, `isMutatingTool`) — this is app-side metadata only; `buildToolsParam` still sends just `name`/`description`/`inputSchema`. Recurrence schemas are flat/wizard-style — friendlier to an LLM than the internal discriminated union. |
